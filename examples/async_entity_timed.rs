...
async fn main() {
    ...
    let timed = "2014-11-28T21:00:09-09:00".parse::<DateTime<FixedOffset>>().unwrap();
    ...
    let edn_body = client.entity_timed(person1.crux__db___id.serialize(), None, Some(timed)).await.await;
    println!("\n Edn Body = {:#?}", edn_body);
    // Edn Body = Map(
    //     Map(
    //         {
